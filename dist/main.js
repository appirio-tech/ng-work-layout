...
$templateCache.put("views/layout-header.directive.html","<ul class=\"flex center middle\"><li class=logo><a ng-home-link=ng-home-link href=\"{{ vm.homeHref }}\" ng-if=\"vm.userType != \'member\'\" class=clean><img src=/images/asp-logo.svg></a><img src=/images/asp-logo.svg ng-if=\"vm.userType == \'member\'\"></li><li class=app-name><a ng-if=\"vm.showAppName &amp;&amp; vm.userType == \'customer\'\" ui-sref=\"timeline({workId: vm.workId})\"><h4>{{vm.appName}}</h4></a><a ng-if=\"vm.showAppName &amp;&amp; vm.userType == \'copilot\'\" ui-sref=\"copilot-project-details({ id: vm.workId })\"><h4>{{vm.appName}}</h4></a><a ng-if=\"vm.showAppName &amp;&amp; vm.userType == \'member\'\"><h4>Design Collaboration</h4></a></li><li><ul ng-if=\"vm.userType != \'member\'\" class=links><li ng-show=vm.loggedIn class=dashboard><a ng-if=\"vm.userType == \'customer\'\" ui-sref=view-work-multiple>Dashboard</a><a ng-if=\"vm.userType != \'customer\'\" ui-sref=copilot-projects>Dashboard</a></li><li ng-show=vm.loggedIn class=projects><button focus-on-click=focus-on-click class=clean>Projects <span class=caret>&dtrif;</span></button><project-drop-down user-type=\"{{ vm.userType }}\" class=\"drop-down transition elevated\"></project-drop-down></li><li ng-hide=vm.loggedIn class=login><a ui-sref=login class=\"button hollow\">Log in</a></li><li ng-show=vm.loggedIn class=profile><button focus-on-click=focus-on-click class=clean><avatar avatar-url={{vm.userAvatar}}></avatar></button><user-drop-down class=\"drop-down transition elevated\"></user-drop-down></li><li ng-show=vm.loggedIn class=notifications><button type=button focus-on-click=focus-on-click class=clean><div ng-class=\"{ danger: vm.unreadCount &gt; 0 }\" class=notification>{{ vm.unreadCount || 0 }}</div></button><message-drop-down ng-if=\"vm.userType == \'customer\'\" subscriber-id=\"{{ vm.subscriberId }}\" class=\"drop-down transition elevated\"></message-drop-down><message-drop-down ng-if=\"vm.userType != \'customer\'\" subscriber-id=\"{{ vm.subscriberId }}\" user-type=copilot class=\"drop-down transition elevated\"></message-drop-down></li></ul><ul ng-if=\"vm.userType == \'member\'\" class=links><li ng-show=vm.loggedIn class=profile><avatar avatar-url={{vm.userAvatar}}></avatar></li></ul></li></ul>");
$templateCache.put("views/layout-project-nav.directive.html","<ul ng-if=\"vm.userType == \'customer\'\"><li ng-class=\"{active: vm.activeLink == \'timeline\'}\"><a ui-sref=\"timeline({ workId: vm.workId })\">Timeline</a></li><li ng-class=\"{active: vm.activeLink == \'submissions\'}\"><a ui-sref=\"step({ projectId: vm.workId, stepId: vm.currentStepId })\">Submissions</a></li><li ng-class=\"{active: vm.activeLink == \'messaging\'}\"><a ui-sref=\"messaging({ id: vm.workId, threadId: vm.threadId })\">Messaging</a></li><li ng-class=\"{active: vm.activeLink == \'project-details\'}\"><a ui-sref=\"project-details({ id: vm.workId })\">Project details</a></li></ul><ul ng-if=\"vm.userType == \'copilot\'\"><li ng-class=\"{active: vm.activeLink == \'copilot-project-details\'}\"><a ui-sref=\"copilot-project-details({ id: vm.workId })\">Project details</a></li><li ng-class=\"{active: vm.activeLink == \'copilot-messaging\'}\"><a ui-sref=\"copilot-messaging({ id: vm.workId, threadId: vm.threadId })\">Messaging</a></li><li ng-class=\"{active: vm.activeLink == \'copilot-submissions\'}\"><a ui-sref=\"step({ projectId: vm.workId, stepId: vm.currentStepId })\">Submissions</a></li><li ng-class=\"{active: vm.activeLink == \'copilot-status-reports\'}\"><a ui-sref=\"copilot-status-reports({ id: vm.workId})\">Status Reports</a></li></ul><ul ng-if=\"vm.userType == \'member\'\"><li ng-class=\"{active: vm.activeLink == \'submissions\'}\"><a ui-sref=\"step({ projectId: vm.workId, stepId: vm.currentStepId })\">Submissions</a></li></ul>");
