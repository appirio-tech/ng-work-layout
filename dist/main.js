...
angular.module("appirio-tech-ng-work-layout").run(["$templateCache", function($templateCache) {$templateCache.put("views/layout-header.directive.html","<a ng-home-link=\"ng-home-link\" href=\"{{ vm.homeHref }}\" class=\"clean logo\">ASP</a><ul class=\"links\"><li><a ui-sref=\"view-work-multiple\">Dashboard</a></li><li class=\"projects\"><button focus-on-click=\"focus-on-click\" class=\"clean\">Projects &dtrif; </button><ul class=\"list\"><li><a ui-sref=\"submit-work\">Create New Project</a></li><li ng-repeat=\"project in vm.projects\"><a ui-sref=\"timeline({ workId: project.id })\">{{ project.name }}</a></li></ul></li><li ng-hide=\"vm.loggedIn\"><a ui-sref=\"login\">Log in</a></li><li ng-show=\"vm.loggedIn\"><a ui-sref=\"view-work-multiple\" class=\"handle\"><avatar></avatar></a></li><li ng-show=\"vm.loggedIn\" class=\"notifications\"><button type=\"button\" focus-on-click=\"focus-on-click\" class=\"clean\"><div ng-show=\"vm.unreadCount &gt; 0\" class=\"notification\">{{ vm.unreadCount }}</div></button><div class=\"popup list\"><threads subscriber-id=\"{{ vm.subscriberId }}\"></threads></div></li></ul><h1 class=\"app-name\">{{ vm.appName }}</h1>");
